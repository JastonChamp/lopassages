document.addEventListener('DOMContentLoaded', () => {
  ...
  fetch('passages.json')
    ...
    .catch(error => {
      ...
      document.getElementById('page').innerHTML = `
        <p>Oops! Stories didn’t load. Error: ${error.message}</p>
        <p>Ensure 'passages.json' is in the same folder and try a local server (e.g., 'python -m http.server 8000').</p>
      `;
      // Fallback to hardcoded passages
      passages = [{
        "title": "Sample Story",
        "text": "This is a <span class='highlight'>s</span>ample <span class='highlight'>t</span>ext to test display.",
        "image": "images/sample.png"
      }];
      document.getElementById('total-stories').textContent = 1;
      document.documentElement.style.setProperty('--total', 1);
      displayPassage(0);
    });
  ...
  function displayPassage(index) {
    ...
    page.innerHTML = `
      ...
      <img id="passage-image" src="${passage.image}" alt="Story Image" onerror="this.style.display='none';">
    `;
    ...
    updateNavigationButtons();
    console.log('Displayed passage:', passage.title); // Debug log
  }
